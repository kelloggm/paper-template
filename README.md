...
1. use GitHub to create a new, private copy of this repository (this is a
template repository).
Name it after the topic of the paper, with `-paper` appended to end. For
example, the RLC paper (FSE 2021) is `resource-leak-paper`. Call this new
name $NAME for the rest of these instructions.
...
Do not add your experimental scripts or anything else that might need to go
into your paper's artifact to the paper repository. Generally, you want to
keep the paper repository private (no one else needs to see your todo
comments!), but you will eventually want to make the artifact repository
public (usually, it can be public from the start).
...
* `make view`: this will build the paper and then open it (using the
  system's `open` command) for viewing
* `make onefile.tex`: creates a single `.tex` file containing the paper,
  suitable for submission to the publisher's website (i.e., with comments
  removed, etc.)
...
Overleaf suppresses some errors, so we recommend that you also look at the
output of `make`.

By default, Overleaf will show you a PDF with broken citations.  You have
two options:

1. Make Overleaf produce correct citations.
   * Change one line in `Makefile` from `pdf-ignore-undefined:
     plume-bib-update` to `pdf-ignore-undefined:`.  That is, remove the
     use of `plume-bib-update`.
   * Periodically run `make copy-plume-bib` and commit and push the
     changes, including any files in the `plume-bib` subdirectory.  To make
     changes within the `plume-bib` subdirectory, please submit a pull
     request to <https://github.com/mernst/plume-bib>.

2. Ignore the problem when editing in Overleaf.  Run `make` in this
